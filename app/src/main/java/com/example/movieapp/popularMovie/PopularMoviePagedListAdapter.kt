...
class PopularMoviePagedListAdapter(public val context: Context):PagedListAdapter<Movie,RecyclerView.ViewHolder>(MovieDiffcallback()) {
    ...
    class MovieItemViewHolder(view: View):RecyclerView.ViewHolder(view){
        fun bind(movie:Movie?,context : Context){
            var title:TextView = itemView.findViewById(R.id.cv_movie_title)
            var releaseDate:TextView = itemView.findViewById(R.id.cv_movie_release_data)
            var poster:ImageView = itemView.findViewById(R.id.cv_iv_movie_poster)
            val movieposter = POSTER_BASE_URL+movie?.posterPath
            Glide.with(context).load(movieposter).into(poster)
